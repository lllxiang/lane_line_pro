                           ...
                           cv::Mat & img_ipm)
{
    ...
    if (camera_type == "weishi")
    {
        points_distort.push_back(cv::Point2f(140.0,250.0));
        points_distort.push_back(cv::Point2f(465.0,257.0));
        points_distort.push_back(cv::Point2f(533.0,351.0));
        points_distort.push_back(cv::Point2f(110.0,322.0));
    }
    else if(camera_type == "senyun")
    {
        points_distort.push_back(cv::Point2f(180.0,220.0));
        points_distort.push_back(cv::Point2f(494.0,200.0));
        points_distort.push_back(cv::Point2f(475.0,320.0));
        points_distort.push_back(cv::Point2f(154.0,318.0));
    }
    ...
    if (debug)
    {
        cv::Mat inv_H = cv::findHomography( wordPoints2D, points_undistort);
        std::cout<<"H"<<H<<std::endl;
        std::cout<<"inv_H"<<inv_H<<std::endl;
    }
...
bool get_mask_img(cv::Mat & src,cv::Mat &dst, int id)
{
    for(int h=0;h<src.rows; ++h)
    {
        ...
        {
            if (src.at<uchar>(h,w) == id || src.at<uchar>(h,w) == 4) //|| src.at<uchar>(h,w) == 7
            {
    ...
    for (int j = 0; j < plines.size(); ++j)
    {
        ...
        if (alpha >= 45 && alpha <= 135) //左右矩形
        {
            ...
            if (r255_1 < 0.5 && r255_2 > 0.5)
            {
                ...
                r255_1 = calu_255r_in_v_line(line,
                                             ...
                                             10, 3);
                //std::cout << "pos 确认 r255_1= " << r255_1 << std::endl;
                if (r255_1 < 0.1)  //pos neg的右矩形 255占比应该极小
                {
                    is_pos = 1;
                    pos_lines.push_back(line);
                }
            }
            if (r255_1 > 0.5 && r255_2 < 0.5) //neg候选
            {
        ...
        else //上下矩形
        {
            ...
            r255_1 = calu_255r_in_v_line(line,
                                         img_ps_mask_ipm,
                                         midp,
                                         8, 1); //返回上方5像素直线, 255所占的比例。
            r255_2 = calu_255r_in_v_line(line,
                                         img_ps_mask_ipm,
                                         midp,
                                         8, 2);//返回下方5像素直线, 255所占的比例。
            if (r255_1 < 0.5 && r255_2 > 0.5)
            {
                //pos候选，需要进一步确定(U型引导线会出现伪pos或者neg)
                r255_1 = calu_255r_in_v_line(line,
                         ...
                         std::vector<cv::Vec4f> & hough_lines)
{
    ...
    for(int i=0; i< hough_lines.size()/5; i++)
    {
        std::cout <<i<<":"<<hough_lines[i][0]<<std::endl;
        std::cout <<i<<":"<<hough_lines[i][1]<<std::endl;
        std::cout <<i<<":"<<hough_lines[i][2]<<std::endl;
        std::cout <<i<<":"<<hough_lines[i][3]<<std::endl;
    }
                                 ...
                                 cv::Mat H)
{
    ...
    if (line_num > 0)
    {
        ...
        for (int j = 0; j < line_num; ++j)
        {
            ...
            if (y1 > y2)
            {
                dst.push_back(cv::Vec4f(x1,y1,x2,y2)); //bottom点为第一个
            }
            else if(abs(y1 < y2) < 1e-5) //如果水平
            {
                if (x1 < x2) //left点为第一个
                {
